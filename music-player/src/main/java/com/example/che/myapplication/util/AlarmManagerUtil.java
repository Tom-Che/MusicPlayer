...
public class AlarmManagerUtil {
    ...
    public static void set2(Context ctx, Class<? extends BroadcastReceiver> clazz, long time, String cmd) {
        AlarmManager am = getAlarmManager(ctx);
        Intent i = new Intent(ctx, clazz);
        i.setAction(cmd);
        PendingIntent pi = PendingIntent.getBroadcast(ctx, 0, i, 0);
        am.cancel(pi);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            am.setExact(AlarmManager.ELAPSED_REALTIME_WAKEUP, SystemClock.elapsedRealtime() + time, pi);
        } else {
